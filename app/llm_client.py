...
class LLMClient:
    ...
    def call_stage1(self, text_chunk: str, title: str, chunk_idx: int, total_chunks: int) -> str | None:
...
- Säilytä konkreettiset komennot, konfiguraatiot ja ratkaisuaskeleet.
""".strip()
        ...
        for attempt in range(1, 4):
            try:
                logger.info("Stage1: kutsutaan mallia %s %s", self.model, osa_info)
                ...
                logger.warning("Stage1 LLM-kutsu epäonnistui, yritys %s/3: %s", attempt, e)
                time.sleep(2 * attempt)

        return None

    def call_stage2_consolidation(self, base_title: str, section_type: str, parts: list[str]) -> str | None:
...
""".strip()
        ...
        for attempt in range(1, 4):
            try:
                logger.info("Stage2: kutsutaan mallia %s (%s)", self.model, section_type)
                response = self.client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": system_prompt},
                ...
                logger.warning("Stage2 konsolidointi epäonnistui, yritys %s/3: %s", attempt, e)
                time.sleep(2 * attempt)
    ...
    def _section_instruction(self, section_type: str) -> str:
        if section_type == "yhteenveto":
            return (
                ...
                "Poista toisteisuus. Säilytä tärkeimmät päätelmät, ratkaisut ja huomiot."
            )
        if section_type == "koodi":
            return (
                "Yhdistä relevantit ja toimivimmat koodit, komennot ja konfiguraatiot. "
                "Poista duplikaatit. Jos useita versioita samasta ratkaisusta esiintyy, "
